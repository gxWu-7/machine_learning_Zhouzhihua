...
class InformationEntropyDecisionTree(object):
    def __init__(self,
                 is_leaf,
                 sub_trees,
                 attribute=None,
    ...
    def get_discontinuous_entropy_gain(df: pd.DataFrame, attribute: str):
        ...
        cur_data_information_entropy = InformationEntropyDecisionTree.compute_information_entropy(df)

        for attribute_value in attribute_values:
            sub_df_k = df[df.loc[:, attribute] == attribute_value]
            p_k = sub_df_k.shape[0] * 1.0 / df.shape[0]
            split_information_entropy += p_k * InformationEntropyDecisionTree.compute_information_entropy(sub_df_k)
    ...
    def get_continuous_entropy_gain_and_attribute_split_value(df: pd.DataFrame, attribute: str):
        ...
        # 选取最优的划分点
        dataset_information_entropy = InformationEntropyDecisionTree.compute_information_entropy(df)
        best_split_value = 0
        ...
        for index in range(0, len(continuous_values) - 1):
            ...
            entropy_less = InformationEntropyDecisionTree.compute_information_entropy(df_less)
            entropy_more_or_equal = InformationEntropyDecisionTree.compute_information_entropy(df_more_or_equal)
    ...
    def generate_decision_tree(df: pd.DataFrame, has_continuous_attribute: bool = True):
        ...
        if has_continuous_attribute:
            return InformationEntropyDecisionTree.generate_decision_tree_with_continuous_attribute(df)
        else:
            return InformationEntropyDecisionTree.generate_decision_tree_without_continuous_attribute(df)
    ...
    def generate_decision_tree_with_continuous_attribute(df: pd.DataFrame):
        ...
        if len(labels) == 1:
            return InformationEntropyDecisionTree(is_leaf=True,
        ...
        for attribute in attributes:
            ...
            if df.loc[:, attribute].dtype == np.float or df.loc[:, attribute].dtype == np.int:
                ...
                # 如果该属性取值为非连续值
                cur_entropy_gain = InformationEntropyDecisionTree.get_discontinuous_entropy_gain(df, attribute)
        ...
        # 开始划分树节点
        tree = InformationEntropyDecisionTree(is_leaf=False,
        ...
        if attribute_is_continuous:
            ...
            tree.sub_trees[split_less] \
                = InformationEntropyDecisionTree.generate_decision_tree_with_continuous_attribute(df_less)
            tree.sub_trees[split_more_or_equal] \
                = InformationEntropyDecisionTree.generate_decision_tree_with_continuous_attribute(df_more_or_equal)
        else:
            # 使用离散值属性划分
            ...
            for attribute_value in best_split_attribute_values:
                ...
                tree.sub_trees[attribute_value] \
                    = InformationEntropyDecisionTree.generate_decision_tree_without_continuous_attribute(sub_df_drop_attribute)
